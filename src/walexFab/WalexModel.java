...
//		}
		...
		(request.getParameter("toCart")!=null){
			int tt=0;
...
//System.out.println(request.getParameter("delete")+" REMOVED");
  ...
  for (Map<String, Object> result : delete_member) { 			  	  
	...
	Session_cart(members_of_map_list(result.get("id")
			.toString(),result.get("total").toString(),result.get("image")
			.toString(),result.get("price").toString(),result.get("input")
			.toString()),request);
  } 
